...
void transferir(conta_t *origem, conta_t *destino, double valor) {
    /*Bloqueando primeiramente a conta com maior id, garantindo assim, com que uma mesma Id consiga
    iniciar uma operação sem antes finalizar*/
    if (origem->id > destino->id) 
    {
        /*Caso a ID de origem seja maior que a ID de destino, da um lock na origem e depois no destino*/
        pthread_mutex_lock(&origem->mutex);
        pthread_mutex_lock(&destino->mutex);
    }
    else
    {
        /*Mesma lógica do if porém, invertida*/
        pthread_mutex_lock(&destino->mutex);
        pthread_mutex_lock(&origem->mutex);
    }
    ...
    /*Desbloqueando os mutexes para a próxima operação*/
